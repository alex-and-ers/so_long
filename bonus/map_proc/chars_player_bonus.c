...
static int	proper_chars_check(char **map, int i, int j)
{
	...
	if (
		...
		|| map[i][j] == coll || map[i][j] == space || map[i][j] == 'U'
	)
		return (1);
	err_msg("You can use only ['E''1''P''C''0''U'] chars in your map!");
	return (0);
}
